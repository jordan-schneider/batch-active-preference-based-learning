...
def information_objective_psi(psi_set, w_samples, delta_samples):
    ...
    if (
        delta_samples.sum(axis=0) > 0
    ):  # hacky way to say if queries are weak preference queries -- the function had better take query_type as input
        return (
            -1.0
            / M
            * (
                np.sum(p1 * np.log2(M * p1 / p1.sum(axis=0)), axis=0)
                + np.sum(p2 * np.log2(M * p2 / p2.sum(axis=0)), axis=0)
                + np.sum(p_Upsilon * np.log2(M * p_Upsilon / p_Upsilon.sum(axis=0)), axis=0)
            )
        )
    else:
        return (
...
def optimize(simulation_object, w_samples, delta_samples, func):
    ...
    opt_res = opt.fmin_l_bfgs_b(
        func,
        x0=np.random.uniform(low=2 * lower_input_bound, high=2 * upper_input_bound, size=(2 * z)),
        args=(simulation_object, w_samples, delta_samples),
...
def volume(simulation_object, w_samples, delta_samples, continuous: bool = False):
    if continuous:
...
def random_discrete(
    simulation_object,
):  # just in case we want random queries to come from the same query database
    z = simulation_object.feed_size
