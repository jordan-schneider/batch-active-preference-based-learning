...
class World(object):
    ...
    def simple_reward(
        self, trajs=None, lanes=None, roads=None, fences=None, speed=1.0, speed_import=1.0
    ):
        if lanes is None:
            lanes = self.lanes
        if roads is None:
        ...
        for road in roads:
            r = r + theta[2] * road.gaussian(10.0)
...
def playground():
    ...
    world.cars.append(car.UserControlledCar(dyn, [-0.17, -0.17, math.pi / 2.0, 0.0], color="white"))
    c = car.Car(dyn, [0.1, -0.25, math.pi / 3.0, 0.0], color="red", T=100)
    world.cars.append(c)
    return world
