...
from driver.simulator import (DrivingSimulation, FetchSimulation,
                              LDSSimulation, MujocoSimulation)
...
class Tosser(MujocoSimulation):
    def __init__(self, total_time=1000, recording_time=[200, 1000]):
        super(Tosser, self).__init__(
            name="tosser", total_time=total_time, recording_time=recording_time
        )
        self.ctrl_size = 4
        self.state_size = 5
        self.feed_size = self.ctrl_size + self.state_size
    ...
    def get_features(self):
        ...
        num_of_flips = (
            np.sum(
                np.abs([recording[i][4] - recording[i - 1][4] for i in range(1, len(recording))])
            )
            / (np.pi * 2)
            / 0.33866545
        )
        ...
        dist_to_basket = (
            np.exp(
                ...
                * np.linalg.norm(
                    [
                        np.minimum(
                            np.abs(recording[len(recording) - 1][3] + 0.9),
                            np.abs(recording[len(recording) - 1][3] + 1.4),
                        ),
                        recording[len(recording) - 1][2] + 0.85,
                    ]
                )
            )
            / 0.17801466
